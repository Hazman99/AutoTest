...
class UserBehavior(TaskSet):
    ...
    def login(self):
        ...
        "username": "LarissaWylie", # Ganti dengan username yang valid
        "password": "RLS71GOH8GF" # Ganti dengan password yang valid
        })
        if response.status_code == 200: self.token = response.json().get("access") # Ambil token akses
        else:
            print("Login failed:", response.text)@task(1)
    ...
    def get_course_contents(self, course_id):
        headers = {"Authorization": f"Bearer {self.token}"}
        response = self.client.get(f"/courses/{course_id}/contents",
        headers=headers)
        if response.status_code == 200: self.contents = response.json() # Simpan daftar konten
        # print("Course Contents:", self.contents)
        if self.contents:self.content_id = self.contents[0]['id'] # Ambil ID konten pertama
        self.post_comment(self.content_id)
        def post_comment(self, content_id): headers = {"Authorization": f"Bearer {self.token}"}
        comment_data = {"comment": "This is a test comment."}
        response = self.client.post(f"/contents/{content_id}/comments",
